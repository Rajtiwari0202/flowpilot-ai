...
function createPostgresRepository({
  ...
  async function write(store) {
    ...
    await sql.begin(async (tx) => {
      await tx`
        select pg_advisory_xact_lock(721904)
      `;

      await tx`
        truncate table public.flowpilot_records
        restart identity
      `;
      ...
      for (const collection of COLLECTIONS) {
        for (const item of normalized[collection]) {
          ...
          if (!recordId) {
            throw new Error(
              `Missing id in ${collection} record`
            );
          }

          await tx`
            insert into public.flowpilot_records (
              collection,
              record_id,
              user_id,
              data
  ...
  return {
    ...
    async read() {
      ...
      const rows = await sql`
        ...
        from public.flowpilot_records
        order by created_at asc
      `;

      if (!rows.length) {
        console.log(
          "🌱 No records found, seeding database..."
        );
