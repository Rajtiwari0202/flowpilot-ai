...
* **Server Component Architecture**: The modular subcomponents under `components/landing/` do not require interactive client state and are built as Server Components. Only `Navbar` uses `"use client"` to handle client transitions, ensuring minimal JS hydration.
