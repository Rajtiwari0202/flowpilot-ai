...
export function SolutionSection() {
  ...
  return (
    <section className="bg-slate-50 py-16 px-6 sm:px-12 max-w-6xl mx-auto border-t border-slate-200" id="how-it-works" aria-label="Solutions Workflow">
        ...
        {steps.map((step, index) => {
          ...
          return (
            <div className="flex flex-col items-center text-center relative" key={step.title}>
              <div className="size-12 rounded-full border border-slate-200 bg-white flex items-center justify-center shadow-sm text-blue-600 relative z-10">
                <Icon size={18} aria-hidden="true" />
              </div>
              <h3 className="font-bold text-xs text-slate-900 mt-4">{step.title}</h3>
              <p className="text-[10px] text-slate-500 mt-1 leading-normal max-w-[120px]">{step.desc}</p>
              
              {/* Connection Arrows between steps */}
              {index < steps.length - 1 && (
                <div className="hidden lg:block absolute top-6 left-[calc(50%+1.5rem)] right-[calc(-50%+1.5rem)] h-px bg-slate-200 z-0"></div>
              )}
            </div>
          );
        })}
