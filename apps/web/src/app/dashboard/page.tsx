...
import { Brand, Banner } from "@/components/AuthComponents";

import { Page, User, Business, WorkflowRow, Lead, Approval, Integration, ActivityRow, Template, SystemStatus, Dashboard } from "./types";
...
export default function DashboardShell() {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      // If validation fails, drop session state and redirect to login
        ...
        <aside className="w-56 shrink-0 border-r border-slate-200 dark:border-[#171717] bg-white dark:bg-[#0A0A0A] p-3">
          ...
          <div className="mt-8 border-t border-slate-200 dark:border-[#171717] pt-4">
            <div className="rounded-md bg-slate-100 dark:bg-[#0E0E0E] border border-slate-200/60 dark:border-[#1C1C1C] p-2.5 text-[11px]">
              <div className="font-bold uppercase tracking-wider text-[9px] text-zinc-500 dark:text-zinc-505 mb-0.5">
            ...
            {isSandbox && <button className="mt-3 w-full text-left text-[11px] font-semibold text-orange-650 dark:text-orange-500 hover:underline" onClick={() => { window.sessionStorage.setItem("flowpilot_skip_sandbox", "1"); logout(); }} type="button">Create your own workspace</button>}
          </div>
        </aside>
